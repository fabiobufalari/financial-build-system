...
// Mock data
const mockAccountsPayable: AccountPayable[] = [
  {
    id: '1',
    invoiceNumber: 'INV-2025-001',
    supplier: 'ABC Materiais de Construção',
    description: 'Materiais para Projeto Residencial',
    amount: 12500,
    dueDate: '2025-06-15',
    ...
    notes: 'Pagamento parcelado em 2x'
  },
  {
    id: '2',
    invoiceNumber: 'INV-2025-002',
    supplier: 'XYZ Serviços Especializados',
  ...
  },
  {
    ...
    supplier: '123 Transportes e Logística',
  ...
  },
  {
    ...
    supplier: 'Consultoria em Projetos SA',
  ...
  },
  {
    ...
    supplier: 'Segurança e Proteção Ltda',
...
const mockMonthlyPayables = [
  { month: 'Jan', amount: 45000 },
  { month: 'Fev', amount: 38000 },
  { month: 'Mar', amount: 52000 },
  { month: 'Abr', amount: 49000 },
  { month: 'Mai', amount: 55000 },
  { month: 'Jun', amount: 57100 }
];

// Dados de aging (vencimento)
const mockAgingData: AgingSummary[] = [
  { range: 'A vencer', amount: 36500, percentage: 64, color: '#10B981' },
  { range: '1-15 dias', amount: 5800, percentage: 10, color: '#FBBF24' },
  { range: '16-30 dias', amount: 8500, percentage: 15, color: '#F97316' },
  { range: '> 30 dias', amount: 6300, percentage: 11, color: '#EF4444' }
];

// Dados por categoria
const mockCategoryData: CategorySummary[] = [
  { name: 'Materiais de Construção', value: 22500, color: '#3B82F6' },
  { name: 'Serviços Especializados', value: 8500, color: '#8B5CF6' },
  { name: 'Equipamentos', value: 9800, color: '#EC4899' },
  { name: 'Transporte', value: 3500, color: '#F97316' },
  { name: 'Consultoria', value: 4500, color: '#10B981' },
  { name: 'Outros', value: 8300, color: '#6B7280' }
];
...
const AccountsPayablePage = () => {
  ...
  const fetchAccountsPayable = async () => {
    try {
      ...
      calculateSummary(mockAccountsPayable);
      setError(null);
    } catch (err) {
  ...
  const calculateSummary = (data: AccountPayable[]) => {
    const summary = data.reduce((acc, account) => {
      acc.totalPayable += account.amount;
  ...
  const sortAccounts = (a: AccountPayable, b: AccountPayable) => {
    let fieldA: any = a[sortField as keyof AccountPayable];
    let fieldB: any = b[sortField as keyof AccountPayable];
  ...
  const exportData = () => {
    const csvContent = [
      ['ID', 'Nº Fatura', 'Fornecedor', 'Descrição', 'Valor', 'Data Vencimento', 'Data Emissão', 'Status', 'Método Pagamento', 'Data Pagamento', 'Categoria', 'Projeto', 'Observações'].join(','),
  ...
  return (
    <div className="p-6 space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 border border-gray-200">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-xl font-semibold text-gray-900">{formatCurrency(summaryData.totalPayable)}</p>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 border border-gray-200">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-xl font-semibold text-green-600">{formatCurrency(summaryData.paidAmount)}</p>
            </div>
          ...
          <p className="text-xs text-gray-500 mt-2">
            {accounts.filter(a => a.status === 'paid').length} faturas pagas
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Gráfico de Contas a Pagar por Mês */}
        <div className="lg:col-span-2 bg-white rounded-lg shadow-sm p-4 border border-gray-200">
          <h2 className="text-lg font-medium text-gray-900 mb-4">Contas a Pagar por Mês</h2>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={mockMonthlyPayables}
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-grow">
          <input
            ...
            placeholder="Buscar fatura, fornecedor..."
      ...
      {loading && !accounts.length ? (
        ...
        <div className="bg-gray-50 border border-gray-200 text-gray-700 px-4 py-8 rounded-lg text-center">
          {searchTerm || filterStatus.length > 0 || filterCategory.length > 0 ? 
            ...
            "Nenhuma conta a pagar registrada no período selecionado."}
        </div>
      ) : (
        <>
